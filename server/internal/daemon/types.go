...
// ProjectEnvironmentData is delivered only in daemon claim payloads. Secrets
// are intended for subprocess environment injection; prompts/context files must
// render only Name, Kind, and Connection.
...
type Task struct {
	...
	ProjectEnvironments      []ProjectEnvironmentData `json:"project_environments,omitempty"`        // allowlisted project envs with plaintext secrets for subprocess env injection
